...
1. Work within **one package at a time** unless cross-package change is
   justified
...
**Never Do:** destructive git, modify outside assigned package, edit versions
manually, weaken types, skip hooks, commit secrets

**Ask First:** cross-package deps, new external deps, CI/CD changes,
`@eserstack/standards` changes, publishing config

**Always Do:** run validate, follow existing patterns, write tests, use explicit
extensions, keep functions pure

## References

See [rules.md](references/rules.md) for file ownership, cross-package protocol,
and detailed safety rules.
