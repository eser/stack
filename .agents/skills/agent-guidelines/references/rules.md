...
Rule: Stay within one package per task. Follow package conventions from
`eserstack-monorepo` skill.
...
Rule: These actions are **strictly prohibited**. No exceptions without explicit
user approval.

- Run destructive git commands (`checkout`, `commit`, `reset`, `push`, `rebase`,
  `stash`, `tag`)
...
- Cross-package dependency changes (adding imports between `@eserstack/*`
  packages)
...
- Run `deno task cli ok` — full monorepo validation (Deno + Go). `make ok` is Go
  only; it does not cover the TypeScript side.
...
1. **Justify** — Explain why multiple packages must change together.
   Single-package changes are preferred.
...
4. **Test** — Run `deno task cli ok` after ALL changes (validates entire
   monorepo)
