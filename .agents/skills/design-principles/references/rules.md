...
Rule: Prefer pure (stateless) functions over stateful ones. Easier to reason
about, test, and reuse.
...
Rule: Keep functions short and focused on single task. Easier to understand,
test, and reuse.
...
Rule: Prefer template strings over concatenation. More readable, allows embedded
expressions.
...
Rule: Separate data and methods. Use plain objects for data, classes only for
stateful services.
...
Rule: Avoid side effects in non-pure functions. Store state closest to where
used.
...
Rule: Avoid code executed at module scope. Wrap in lazy initialization functions
instead.
...
Refactoring approach: When encountering module-level side effects, wrap in
getter functions that compute on first call and cache the result. Use promises
