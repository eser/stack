...
Rule: Follow strict hexagonal architecture with clear separation of concerns.
...
Rule: Follow consistent directory structure. Avoid `internal/`, `api/`,
...
Rule: Always check and handle errors explicitly. Wrap errors for traceability.
...
Rule: Use appropriate log levels by layer. Never log sensitive information.
...
Rule: Use table-driven tests with parallel execution. Run with race detection.
...
Rule: Use OpenTelemetry for distributed tracing, metrics, and structured
logging.
...
Rule: Implement defensive patterns for external calls.
...
Rule: Write short, focused functions. Prefer composition over inheritance.
...
Rule: Prefer `encoding/json/v2` (jsonv2) over `encoding/json` wherever possible
for JSON marshaling/unmarshaling.

**Guidelines:**

- Import `encoding/json/v2` instead of `encoding/json`
- Use jsonv2 struct tags and features (e.g., `omitzero`, `inline`, format
  options)
