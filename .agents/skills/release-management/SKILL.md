...
- **patch:** bug fixes, docs, deps. **minor:** new features. **major:** breaking
  changes
...
**"I'll just edit one package's version"** No. Use the version-bump script.
Manual edits break synchronization.

**"Skip the changelog, it's a small fix"** No. Every release needs a changelog
entry — `release-gate` fails the tag run when `CHANGELOG.md` has no section for
the tagged version.

**"The publish failed, I'll just retag the same version"** Only while nothing
has been published. JSR is immutable (yank only) and npm answers a republish
with 403, so once `publish` or `publish-ajan` has written anything, re-run the
failed jobs (infrastructure error) or cut a new patch version (code fix).

## References

See [rules.md](references/rules.md) for full release checklist and hotfix
process.
