...
- **NEVER** use `--allow-all` in production code (only acceptable in `scripts/`
  and `*.test.ts`)
...
Rule: All secrets MUST be in environment variables, never in config files or
code.
...
Rule: Ensure all security settings are properly configured for production.
...
Rule: Validate URLs before making requests. Block internal IP ranges.
...
Rule: Sanitize error responses in production. Never expose stack traces or
internal details.
...
Rule: Clearly separate development and production configurations.
...
Rule: Use secure defaults. Require explicit opt-out for less secure options.
