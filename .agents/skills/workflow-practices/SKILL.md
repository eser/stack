...
4. Run `deno task cli ok` before considering work complete (`make ok` is Go
   only)
...
**"I'll revert the user's code to fix this"** No. Never overwrite intentional
user changes. Work with their code.

**"Let me just commit these changes"** No. Never create commits unless
explicitly requested by the user.

**"I'll skip validate, it's a small change"** No. Run `deno task cli ok` before
considering any work complete — it covers both languages. `make ok` is Go only.
