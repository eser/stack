...
Rule: Follow a consistent workflow for task execution.
...
Rule: NEVER revert, undo, or overwrite code that user intentionally wrote.
...
Rule: NEVER use `git checkout`, `git restore`, or any git command to revert file
contents. Parallel tasks run with parallel agents, so git-based reverting can
destroy work from other agents. Always undo changes using Edit operations.
...
Rule: Do NOT create git commits unless explicitly requested by user.
...
Rule: Never change native tooling without explicit permission.
...
Rule: When uncertain, ask or state the uncertainty. Never guess.
...
Rule: Fully implement all requested functionality. No partial implementations.
...
function getUserName(
  ...
  return includeTitle && user.title ? `${user.title} ${user.name}` : user.name;
}
```
