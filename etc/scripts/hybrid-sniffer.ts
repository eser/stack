...
//
// Usage:
//   deno run --allow-read etc/scripts/hybrid-sniffer.ts pkg/@eserstack/<package>
//   deno run --allow-read etc/scripts/hybrid-sniffer.ts pkg/@eserstack/cs pkg/@eserstack/cache
//
...
if (args.length === 0) {
  console.error(
    "Usage: deno run --allow-read etc/scripts/hybrid-sniffer.ts <pkg-path> [...]",
  );
  Deno.exit(1);
}
