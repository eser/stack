<#
.SYNOPSIS
  noskills-server install script for Windows.
...
.PARAMETER InstallDir
  Where to place the binaries. Defaults to $env:LOCALAPPDATA\Programs\eser\bin,
  a per-user location chosen so the script never needs Administrator. Overridable
  with -InstallDir or the INSTALL_DIR environment variable, mirroring install.sh.
...
.PARAMETER ArchivePath
  ...
  the seam the CI job uses to exercise the installer against the archive built
  from the current tree, rather than only against whatever is already published.

.EXAMPLE
  irm https://raw.githubusercontent.com/eser/stack/main/etc/scripts/install-noskills.ps1 | iex

.EXAMPLE
  .\install.ps1 -Version 4.1.58 -InstallDir C:\tools\eser
#>
...
try {
  if ($ArchivePath) {
    ...
    $archive = "noskills_${Version}_Windows_${archName}.zip"
    ...
    # Checksum verification, matching install.sh. checksums.txt is a
    ...
    try {
      ...
      if ($line) {
        ...
        Write-Warning "checksums.txt has no entry for $archive; skipping verification."
      }
    }
