...
const help = `lime-init

Initialize a new cool lime project. This will create all the necessary files for a
new project.
...
console.log(
  colors.bgRgb8(
    colors.black(colors.bold(" 🍋 cool lime: The next-gen web framework. ")),
    121,
  ),
);
console.log();
...
if (flags._.length !== 1) {
  const userInput = prompt("Project Name", "lime-project");
  if (!userInput) {
    error(help);
  }
...
console.log(
  "%cLet's set up your new cool lime project.\n",
  "font-weight: bold",
);
...
try {
  const faviconArrayBuffer = await fetch(
    "https://coollime.deno.dev/favicon.ico",
  )
...
Your new cool lime project is ready to go. You can follow the cool lime "Getting
Started" guide here: https://coollime.deno.dev/docs/getting-started
