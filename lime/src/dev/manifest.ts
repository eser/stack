import { extname, join, posix } from "./deps.ts";
...
export async function generate(directory: string, manifest: Manifest) {
  ...
  const normalizedIslands = new Map<string, string>();
  for (let i = 0; i < islands.length; i++) {
    const file = islands[i];
    const specifier = toImportSpecifier(file);
    const identifier = specifierToIdentifier(file, used);
    normalizedIslands.set(specifier, identifier);
  }

  const output = `// DO NOT EDIT. This file is generated by cool lime.
...
${
    Array.from(normalizedIslands.entries()).map(([specifier, identifier]) =>
      `import * as $$${identifier} from "${specifier}";`
    )
      .join("\n")
  }
