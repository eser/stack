...
async function testBuild(
  ...
  const outDir = options.outDir ?? path.join(fixture, subDirPath, "_lime");
  ...
  try {
    ...
    await Deno.remove(path.join(fixture, subDirPath, "_lime"), {
      recursive: true,
    });
  }
}
...
Deno.test("pass target options", async () => {
  ...
  const txt = await Deno.readTextFile(
    path.join(fixture, "_lime", "island-counter_default.js"),
  );
