...
Deno.test({
  ...
  async fn(t) {
    ...
    await t.step("check deno.jsonc", async () => {
      ...
      assertEquals(json.exclude, ["**/_lime/*"]);
    });
...
Deno.test({
  ...
  async fn(t) {
    ...
    const files = [
      ...
      "/subdirectory/subsubdirectory/manifest.gen.ts",
    ];
...
Deno.test({
  name: "lime-init loads env variables",
  async fn(t) {
    // Preparation
    const tmpDirName = await Deno.makeTempDir();
    ...
    await t.step("build code and start server again", async () => {
      ...
      const { serverProcess, lines, address, output } = await startLimeServer({
