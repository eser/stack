...
Deno.test("throws when passing non-jsx children to an island", async (t) => {
  await withFakeServe(
    ...
    async (server) => {
      ...
      assertSelector(doc2, ".lime-error-page");
      ...
      await t.step("should not throw on valid children", async () => {
        ...
        assertNotSelector(doc2, ".lime-error-page");
      });
    },
  );
});
