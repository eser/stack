...
Deno.test("/ page prerender", async () => {
  const resp = await handler(new Request("https://coollime.deno.dev/"));
  assert(resp);
  assertEquals(resp.status, Status.OK);
  assertEquals(resp.headers.get("content-type"), "text/html; charset=utf-8");
  assertEquals(resp.headers.get("server"), "cool lime test server");
...
Deno.test("/props/123 page prerender", async () => {
  const resp = await handler(
    new Request("https://coollime.deno.dev/props/123"),
  );
  assert(resp);
  assertEquals(resp.status, Status.OK);
  assertEquals(resp.headers.get("content-type"), "text/html; charset=utf-8");
  ...
  assertEquals(data, {
    "params": { "id": "123" },
    "url": "https://coollime.deno.dev/props/123",
    "route": "/props/:id",
    "state": { "root": "root_mw" },
  });
});
...
Deno.test("/greet/[name] page prerender", async () => {
  const resp = await handler(new Request("https://coollime.deno.dev/bar"));
  assert(resp);
  assertEquals(resp.status, Status.OK);
  assertEquals(resp.headers.get("content-type"), "text/html; charset=utf-8");
  const body = await resp.text();
...
Deno.test("/api/head_override - HEAD", async () => {
  const req = new Request("https://coollime.deno.dev/api/head_override", {
    method: "HEAD",
  });
...
Deno.test("/api/get_only - HEAD fallback", async () => {
  const req = new Request("https://coollime.deno.dev/api/get_only", {
    method: "HEAD",
  });
  const resp = await handler(req);
  assert(resp);
...
Deno.test("/intercept - GET html", async () => {
  const req = new Request("https://coollime.deno.dev/intercept", {
...
Deno.test("/intercept_args - GET html", async () => {
  const req = new Request("https://coollime.deno.dev/intercept_args", {
    headers: { "accept": "text/html" },
  });
  const resp = await handler(req);
  assert(resp);
...
Deno.test("/status_overwrite", async () => {
  const req = new Request("https://coollime.deno.dev/status_overwrite", {
    headers: { "accept": "text/html" },
  });
  const resp = await handler(req);
  assert(resp);
...
Deno.test("/api/get_only - NOTAMETHOD", async () => {
  const resp = await handler(
    new Request("https://coollime.deno.dev/api/get_only", {
      method: "NOTAMETHOD",
    }),
  );
  assert(resp);
...
Deno.test("/api/xyz not found", async () => {
  const resp = await handler(new Request("https://coollime.deno.dev/api/xyz"));
  assert(resp);
...
Deno.test("/static page prerender", async () => {
  const resp = await handler(new Request("https://coollime.deno.dev/static"));
  assert(resp);
  assertEquals(resp.status, Status.OK);
  assertEquals(resp.headers.get("content-type"), "text/html; charset=utf-8");
  const body = await resp.text();
  ...
  assertStringIncludes(body, `src="/image.png?__lime_c=`);
  assert(!body.includes("__LIME_ISLAND_PROPS"));
});

Deno.test("/books/:id page - /books/123", async () => {
  const resp = await handler(
    new Request("https://coollime.deno.dev/books/123"),
  );
  assert(resp);
  assertEquals(resp.status, Status.OK);
  assertEquals(resp.headers.get("content-type"), "text/html; charset=utf-8");
...
Deno.test("/books/:id page - /books/abc", async () => {
  const resp = await handler(
    new Request("https://coollime.deno.dev/books/abc"),
  );
  assert(resp);
  assertEquals(resp.status, Status.NotFound);
});
...
Deno.test("/i18n{/:lang}?/lang page - /i18n/lang", async () => {
  const resp = await handler(
    new Request("https://coollime.deno.dev/i18n/lang"),
  );
  assert(resp);
  assertEquals(resp.status, Status.OK);
  assertEquals(resp.headers.get("content-type"), "text/html; charset=utf-8");
...
Deno.test("/i18n{/:lang}?/lang page - /i18n/en/lang", async () => {
  const resp = await handler(
    new Request("https://coollime.deno.dev/i18n/en/lang"),
  );
  assert(resp);
  assertEquals(resp.status, Status.OK);
  assertEquals(resp.headers.get("content-type"), "text/html; charset=utf-8");
...
Deno.test("redirect /pages/lime/ to /pages/lime", async () => {
  const resp = await handler(
    new Request("https://coollime.deno.dev/pages/lime/"),
  );
  assert(resp);
...
Deno.test("no redirect for /pages/////lime", async () => {
  const resp = await handler(
    new Request("https://coollime.deno.dev/pages/////lime"),
  );
  assert(resp);
...
Deno.test("/failure", async () => {
  const resp = await handler(new Request("https://coollime.deno.dev/failure"));
  assert(resp);
...
Deno.test("/foo/:path*", async () => {
  const resp = await handler(
    new Request("https://coollime.deno.dev/foo/bar/baz"),
  );
  assert(resp);
  assertEquals(resp.status, Status.OK);
  const body = await resp.text();
...
Deno.test("static files in custom directory", async () => {
  ...
  const resp = await newRouter(
    new Request("https://coollime.deno.dev/custom.txt"),
  );
  assertEquals(resp.status, Status.OK);
  const body = await resp.text();
...
Deno.test("static file - by file path", async () => {
  const resp = await handler(new Request("https://coollime.deno.dev/foo.txt"));
  assertEquals(resp.status, Status.OK);
  const body = await resp.text();
  ...
  const resp2 = await handler(
    new Request("https://coollime.deno.dev/foo.txt", {
      headers: {
        "if-none-match": etag,
      },
    }),
...
Deno.test("static file - by 'hashed' path", async () => {
  ...
  const resp = await handler(
    new Request("https://coollime.deno.dev/assetsCaching"),
  );
  const body = await resp.text();
  const imgFilePath = body.match(/img id="img-with-hashing" src="(.*?)"/)?.[1];
  assert(imgFilePath);
  assert(imgFilePath.includes(`?__lime_c=${BUILD_ID}`));

  // check the static file is served correctly under its cacheable route
  const resp2 = await handler(
    new Request(`https://coollime.deno.dev${imgFilePath}`),
  );
...
Deno.test("/params/:path*", async () => {
  const resp = await handler(
    new Request("https://coollime.deno.dev/params/bar/baz"),
  );
  assert(resp);
  assertEquals(resp.status, Status.OK);
  const body = await resp.text();
...
Deno.test("/connInfo", async () => {
  const resp = await handler(new Request("https://coollime.deno.dev/connInfo"));
  assert(resp);
  assertEquals(resp.status, Status.OK);
  const body = await resp.text();
...
Deno.test("state in page props", async () => {
  const resp = await handler(
    new Request("https://coollime.deno.dev/state-in-props"),
  );
  assert(resp);
  assertEquals(resp.status, Status.OK);
  assertEquals(resp.headers.get("content-type"), "text/html; charset=utf-8");
...
Deno.test({
  ...
  fn: async () => {
    const resp = await handler(
      new Request("https://coollime.deno.dev/middleware_root"),
    );
    assert(resp);
    assertEquals(resp.status, Status.OK);
    const body = await resp.text();
...
Deno.test({
  ...
  fn: async () => {
    const resp = await handler(
      new Request("https://coollime.deno.dev/layeredMdw/layer2/abc"),
    );
    assert(resp);
    assertEquals(resp.status, Status.OK);
    const body = await resp.text();
    ...
    const resp1 = await handler(
      new Request(
        "https://coollime.deno.dev/layeredMdw/layer2-no-mw/without_mw",
      ),
    );
    assert(resp1);
...
Deno.test({
  ...
  fn: async () => {
    const resp = await handler(
      new Request("https://coollime.deno.dev/layeredMdw/layer2"),
    );
    assert(resp);
    assertEquals(resp.status, Status.OK);
    const body = await resp.text();
...
Deno.test({
  ...
  fn: async () => {
    ...
    const resp = await handler(
      new Request("https://coollime.deno.dev/layeredMdw/layer2/layer3/abc"),
    );
    assert(resp);
    assertEquals(resp.status, Status.OK);
    const body = await resp.text();
    ...
    assertEquals(resp.headers.get("layer3"), "cool lime test server layer3");
...
Deno.test({
  ...
  fn: async () => {
    const resp = await handler(
      new Request("https://coollime.deno.dev/state-middleware/foo"),
    );
    assert(resp);
    assertEquals(resp.status, Status.OK);
...
Deno.test({
  ...
  fn: async () => {
    const resp = await handler(
      new Request(
        "https://coollime.deno.dev/layeredMdw/layer2-with-params/tenant1/resource1",
      ),
    );
    assert(resp);
    assertEquals(resp.status, Status.OK);
...
Deno.test({
  ...
  fn: async () => {
    const resp = await handler(
      new Request(
        "https://coollime.deno.dev/layeredMdw/nesting/acme/production/abc",
      ),
    );
    assert(resp);
    assertEquals(resp.status, Status.OK);
...
Deno.test({
  ...
  fn: async () => {
    const resp = await handler(
      new Request("https://coollime.deno.dev/not_found"),
    );
    assert(resp);
...
Deno.test("middleware destination", async (t) => {
  ...
  await t.step("notFound", async () => {
    const resp = await handler(
      new Request("https://coollime.deno.dev/bar/bar"),
    );
    assert(resp);
...
Deno.test({
  ...
  fn: async () => {
    const resp = await handler(
      new Request(
        "https://coollime.deno.dev/middleware-error-handler",
      ),
    );
    assert(resp);
    assertEquals(resp.status, Status.InternalServerError);
...
Deno.test({
  ...
  fn: async () => {
    const resp = await handler(
      new Request(
        "https://c00l.deno.dev/404-from-middleware",
      ),
    );
    assert(resp);
    assertEquals(resp.status, Status.NotFound);
...
Deno.test({
  ...
  fn: async () => {
    const resp = await handler(
      new Request(
        "https://c00l.deno.dev/404_from_throw",
      ),
    );
    assert(resp);
    assertEquals(resp.status, Status.NotFound);
...
Deno.test({
  ...
  fn: async () => {
    const resp = await handler(
      new Request(
        "https://c00l.deno.dev/404-from-middleware-throw",
      ),
    );
    assert(resp);
    assertEquals(resp.status, Status.NotFound);
...
Deno.test("preloading javascript files", async () => {
  ...
  try {
    ...
    assert(
      preloads.some((url) => url.match(/\/_lime\/js\/.*\/main\.js/)),
      "preloads does not include main.js",
    );
    assert(
      preloads.some((url) => url.match(/\/_lime\/js\/.*\/island-.*\.js/)),
      "preloads does not include island-*.js",
    );
    assert(
      preloads.some((url) => url.match(/\/_lime\/js\/.*\/chunk-.*\.js/)),
...
Deno.test("throw on route export 'handlers' instead of 'handler'", async () => {
...
Deno.test("Resolves routes with non-latin characters", async () => {
  await withFakeServe("./tests/fixture/main.ts", async (server) => {
    ...
    // Check the manifest
    const mod = (await import("./fixture/manifest.gen.ts")).default;
...
Deno.test("Adds nonce to inline scripts", async () => {
  await withFakeServe("./tests/fixture/main.ts", async (server) => {
    ...
    const stateScript = doc.querySelector("#__LIME_STATE")!;
...
Deno.test("pass options in config", async (t) => {
  ...
  await t.step("config.onListen", async () => {
    const { lines, serverProcess, output } = await startLimeServer({
      args: [
        "run",
        "-A",
