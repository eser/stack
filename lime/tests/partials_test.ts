...
Deno.test("reconciles keyed DOM nodes", async () => {
  await withPageName(
    ...
    async (page, address) => {
      ...
      // Check that no element has `data-lime-key` attribute
      const doc = parseHtml(await page.content());
      assertNotSelector(doc, "[data-lime-key]");
    },
  );
});
...
Deno.test("non-partial client navigation", async () => {
  await withPageName(
    ...
    async (page, address) => {
      ...
      await page.evaluate(() => {
        const marker = document.createElement("lime-nav-marker");
      ...
      assertSelector(doc, "lime-nav-marker");
...
Deno.test("allow opting out of client navigation", async () => {
  await withPageName(
    ...
    async (page, address) => {
      ...
      let doc = parseHtml(await page.content());
      assertNotSelector(doc, ".island-a");
      assertNotSelector(doc, "lime-nav-marker");
