...
Deno.test("/static page prerender", async () => {
  const resp = await router(new Request("https://coollime.deno.dev/static"));
  assert(resp);
...
Deno.test("/with-island prerender", async () => {
  const resp = await router(
    new Request("https://coollime.deno.dev/with-island"),
  );
  assert(resp);
  assertEquals(resp.status, Status.OK);
  const body = await resp.text();
...
Deno.test("plugin routes and middleware", async () => {
  const resp = await router(new Request("https://coollime.deno.dev/test"));
  assert(resp);
  assertEquals(resp.status, Status.OK);
  const body = await resp.text();
  assertStringIncludes(
...
Deno.test("plugin middleware multiple handlers", async () => {
  const resp = await router(
    new Request("https://coollime.deno.dev/lots-of-middleware"),
  );
  assert(resp);
  assertEquals(resp.status, Status.OK);
  const body = await resp.text();
...
Deno.test("plugin route no leading slash", async () => {
  const resp = await router(
    new Request("https://coollime.deno.dev/no-leading-slash-here"),
  );
  assert(resp);
  assertEquals(resp.status, Status.OK);
  const body = await resp.text();
...
Deno.test({
  ...
  async fn(t) {
    // Preparation
    const { lines, serverProcess, address } = await startLimeServer({
...
Deno.test("calls buildStart() and buildEnd()", async () => {
  ...
  assertEquals(out, [
    "Plugin a: buildStart",
    "Plugin b: buildStart",
    `Plugin c: ${join("tests", "fixture_plugin_lifecycle", "_lime")}`,
    "Plugin a: buildEnd",
    "Plugin b: buildEnd",
  ]);
});
