...
deno run --allow-all pkg/@eser/ajan/scripts/build.ts
...
ESER_AJAN_LIB_PATH=$(pwd)/pkg/@eser/ajan/dist/aarch64-darwin/libeser_ajan.dylib \
  bun --eval 'import * as ffi from "./pkg/@eser/ajan/ffi/mod.ts"; const lib = await ffi.loadEserAjan(); console.log(lib.symbols.EserAjanVersion()); lib.close();'
...
```bash
deno run --allow-all pkg/@eser/ajan/scripts/build.ts --target=x86_64-linux
```

### All targets (CI)

```bash
deno run --allow-all pkg/@eser/ajan/scripts/build.ts --all
```
...
```bash
deno run --allow-all pkg/@eser/ajan/scripts/build.ts --clean
```
...
```bash
deno run --allow-all pkg/@eser/ajan/scripts/package.ts
```

Override version:

```bash
deno run --allow-all pkg/@eser/ajan/scripts/package.ts --version=1.2.3
```
