 ...
 * cross-runtime abstraction layer `@eser/standards/cross-runtime`.
 *
 * Allowed exclusions:
 * - `@eser/standards/cross-runtime/` — the abstraction layer itself
...
const DENO_PATTERNS: readonly ForbiddenPattern[] = [
  ...
  { pattern: /\bDeno\.open\b/, replacement: "runtime.fs (or @eser/streams)" },
  { pattern: /\bDeno\.exit\b/, replacement: "runtime.process.exit()" },
  {
    pattern: /\bnew Deno\.Command\b/,
    replacement: "runtime.exec.spawn() (or @eser/shell/exec)",
  },
  {
...
export const tool: FileTool = createFileTool({
  ...
  description:
    "Detect direct usage of runtime-specific APIs (use @eser/standards/cross-runtime instead)",
  canFix: false,
  stacks: ["javascript"],
  defaults: {},
      ...
      for (const { pattern, replacement } of DENO_PATTERNS) {
        ...
        issues.push({
          ...
          message: `direct Deno API usage: ${
            match[0]
          } — use @eser/standards/cross-runtime (${replacement})`,
        });
      }
    }
