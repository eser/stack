...
// Lazy FFI singleton — loaded once per process, reused for every kit call.
...
export const ensureLib = (): Promise<void> => {
  if (_libPromise === null) {
    _libPromise = import("@eserstack/ajan/ffi")
      ...
      .catch(() => {
        // Native library unavailable — callers use TS registry fallback.
      });
  }
...
// Eagerly start loading so the handle exists before any test's resource
// sanitizer window opens — prevents false-positive leak detection in tests.
void ensureLib();
