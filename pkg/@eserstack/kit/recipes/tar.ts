...
/**
 * Tarball extraction utility for kit recipes
 *
...
export const extractTarball = async (
  ...
  for await (const entry of untarStream) {
    // Security: Reject symlinks and hardlinks to prevent tarball symlink traversal escapes
    if (
      (entry as unknown as Record<string, unknown>)["type"] === "symLink" ||
      Boolean((entry as unknown as Record<string, unknown>)["linkname"])
    ) {
