 ...
 * Uses @eserstack/bundler directly for bundling operations.
 *
 * Supports two backends:
 * - "deno-bundler": Native Deno.bundle() API (default)
 * - "rolldown": Rolldown bundler via @eserstack/bundler (faster, advanced chunking)
 */
...
import {
  ...
  type BundlerPlugin,
  createBundler,
} from "@eserstack/bundler/backends";
...
/**
 * Convert @eserstack/bundler result to laroux format.
 */
...
export async function bundle(
  ...
  bundlerLogger.debug(`Bundling with @eserstack/bundler (${backend})...`);
  ...
  // Convert sourcemap to boolean for @eserstack/bundler
