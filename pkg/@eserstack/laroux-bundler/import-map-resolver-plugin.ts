 ...
 * 2. Resolves ALL bare imports (not just @eserstack/*)
...
import { tryLoad as tryLoadPackage } from "@eserstack/codebase/package";
  ...
  try {
    ...
    for await (const entry of runtime.fs.readDir(denoDir)) {
      if (entry.isDirectory && entry.name.startsWith(`${packageName}@`)) {
        ...
        // Load package config using @eserstack/codebase (handles deno.json, jsr.json, package.json)
