...
import { renderOnboardingBanner } from "../output/banner.ts";
import { detectMode, stripModeFlag } from "../output/mode.ts";
import { detectAgentTool } from "@eserstack/shell/env";
...
export const main = async (
  ...
  if (mode === "agent") {
    ...
    const ACTIVE_SPEC_PHASES = new Set([
      "DISCOVERY",
    ...
    let specStates: Awaited<ReturnType<typeof persistence.listSpecStates>> = [];
    try {
      specStates = await persistence.listSpecStates(root);
    } catch {
      // Defensive — listSpecStates normally returns [] on IO errors.
      // Fall through to simple message if it ever does throw.
    }

    const hasActiveSpecs = specStates.some((s) =>
      ACTIVE_SPEC_PHASES.has((s as { phase?: string }).phase ?? "")
    );

    if (!hasActiveSpecs) {
      renderOnboardingBanner(ctx, {
        concernCount: selectedConcernIds.length,
