...
describe("IDLE behavioral — AskUserQuestion, concerns, user intent", () => {
  ...
  it("IDLE instruction — zero specs state — says ask what to build", async () => {
    // idle() creates a fresh state with no existing specs → zero-specs branch
    const output = await compiler.compile(idle(), noConcerns, noRules);
    const idleOutput = output as compiler.IdleOutput;
    assertEquals(
      idleOutput.instruction.includes("What do you want to build"),
      true,
    );
  });
...
describe("DISCOVERY_REFINEMENT behavioral", () => {
  ...
  it("contains completeness assessment rule (stage-a)", async () => {
    const output = await compiler.compile(
      inDiscoveryReview(),
      noConcerns,
      noRules,
  ...
  it("tone says 'Structured evaluator' in stage-a", async () => {
    const output = await compiler.compile(
      inDiscoveryReview(),
      noConcerns,
      noRules,
    );
    assertEquals(output.behavioral.tone.includes("Structured evaluator"), true);
  });
});
