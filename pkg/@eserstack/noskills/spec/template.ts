...
// =============================================================================
// Section normalization (for SPEC_PROPOSAL render — handles both shapes)
// =============================================================================
...
/**
 * Normalize a specSections entry to a consistent shape.
 * Handles both the legacy string format and the new SpecSectionDefinition shape.
 */
const normalizeSpecSection = (
  concernId: string,
  ...
  if (typeof raw === "string") {
    return {
      ...
      title: raw,
      placeholder: "_To be addressed during execution._",
      condition: null,
    };
  }
  return raw;
...
/**
 * Check whether a normalized section is relevant given the current classification.
 * Uses the `condition` field when available. Falls back to always-relevant for
 * sections without a condition (null = always show).
 */
const isSectionRelevant = (
  section: NormalizedSection,
  classification: schema.SpecClassification | null,
): boolean => {
  if (section.condition === null) return true;
  if (classification === null) return false;

  const conditionMap: Record<
    string,
    (c: schema.SpecClassification) => boolean
  > = {
  ...
  const condFn = conditionMap[section.condition];
  return condFn !== undefined ? condFn(classification) : true;
};
...
export const renderSpec = (
  ...
  for (const concern of concerns) {
    if (concern.specSections.length > 0) {
      for (const raw of concern.specSections) {
        const section = normalizeSpecSection(concern.id, raw);
        if (!isSectionRelevant(section, classification ?? null)) continue;

        lines.push(`## ${section.title} (${concern.id})`);
        lines.push("");
