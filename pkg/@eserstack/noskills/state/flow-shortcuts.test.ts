...
describe("Happy path: full lifecycle with classification", () => {
  ...
  it("all-false classification hides conditional concern sections only", () => {
    const allFalse = {
      involvesWebUI: false,
      involvesCLI: false,
    ...
    // Conditional sections (involvesWebUI) hidden when flag is false
    assertEquals(md.includes("Design States"), false);
    assertEquals(md.includes("Mobile Layout"), false);
    // Unconditional sections (condition: null) always shown
    assertEquals(md.includes("Contributor Guide"), true);
  });
});
...
describe("Skip classification: approve without classifying", () => {
  ...
  it("null classification hides conditional sections but shows unconditional ones", () => {
    const md = template.renderSpec("test", [], activeConcerns, [], null);

    // Conditional sections (involvesWebUI) are hidden when classification is null
    assertEquals(md.includes("Design States"), false);
    assertEquals(md.includes("Mobile Layout"), false);
    // Unconditional sections (condition: null) are always visible
    assertEquals(md.includes("Contributor Guide"), true);
    assertEquals(md.includes("Public API Surface"), true);
  });
