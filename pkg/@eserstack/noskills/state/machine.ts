...
export const approveDiscoveryReview = (
  ...
  // Jidoka Completeness Gate — every placeholder must be resolved before advancing.
  // N/A without a reason counts as unresolved. Hidden sections are exempt.
  const report = livingSpec.checkSpecCompleteness(state.specState);
  if (!report.canAdvance) {
    const sections = report.unresolvedSections
      .map((s) => `  - ${s.sectionTitle}`)
      .join("\n");
    const decisions = report.pendingDecisions
      .map((d) =>
        `  - ${d.section}: ${d.question} (waiting for ${
          d.waitingFor.join(", ")
        })`
      )
      .join("\n");
    throw new Error(
      "Spec incomplete — cannot advance to SPEC_PROPOSAL.\n" +
        (report.unresolvedSections.length > 0
          ? `\nUnresolved sections:\n${sections}`
          : "") +
        (report.pendingDecisions.length > 0
          ? `\nPending decisions:\n${decisions}`
          : "") +
        "\n\nUse AskUserQuestion to resolve every item. Do not guess, infer, or fill in yourself.",
    );
  }

  assertTransition(state.phase, "SPEC_PROPOSAL");
  return autoClassifyIfMissing({ ...state, phase: "SPEC_PROPOSAL" });
};
...
export const reopenSpec = (
  ...
  return {
    ...
    completedAt: null,
    completionNote: null,
    // Preserve discovery answers for revision; clear refinement sub-state (stale after reopen)
    discovery: {
      ...state.discovery,
      completed: false,
      currentQuestion: 0,
...
export const revisitSpec = (
  ...
  return {
    ...state,
    phase: "DISCOVERY",
    // Preserve discovery answers for revision; clear refinement sub-state (stale after revisit)
    discovery: {
      ...state.discovery,
      completed: false,
      currentQuestion: 0,
...
// =============================================================================
// DISCOVERY_REFINEMENT sub-state (review posture + completeness scoring)
// =============================================================================

/**
 * Detect which sub-stage of DISCOVERY_REFINEMENT the state is in.
 * Exported so compiler.ts and commands/next.ts can both use it without duplication.
 *   stage-a: no posture set yet (completeness assessment + posture selection)
 *   stage-b: posture set, CEO review pending
 *   stage-c: CEO review submitted
 */
export const getDiscoveryRefinementStage = (
  state: schema.StateFile,
): "stage-a" | "stage-b" | "stage-c" => {
  const ref = state.discovery.refinement;
  if (!ref?.reviewPosture) return "stage-a";
  if (!ref.ceoReview) return "stage-b";
  return "stage-c";
};

/**
 * Store or update the completeness score.
 * First call sets both initialCompletenessScore (write-once) and completenessScore.
 * Subsequent calls update completenessScore only — initialCompletenessScore is preserved via ??-guard.
 */
export const setCompletenessScore = (
  state: schema.StateFile,
  score: schema.CompletenessScore,
): schema.StateFile => {
  if (state.phase !== "DISCOVERY_REFINEMENT") {
    throw new Error(`setCompletenessScore: invalid phase ${state.phase}`);
  }
  const existing = state.discovery.refinement;
  return {
    ...state,
    discovery: {
      ...state.discovery,
      refinement: {
        ...existing,
        initialCompletenessScore: existing?.initialCompletenessScore ?? score,
...
/** Set the review posture chosen by the user in DISCOVERY_REFINEMENT stage-a. */
export const setReviewPosture = (
  state: schema.StateFile,
  posture: schema.ReviewPosture,
): schema.StateFile => {
  if (state.phase !== "DISCOVERY_REFINEMENT") {
    throw new Error(`setReviewPosture: invalid phase ${state.phase}`);
  }
  return {
    ...state,
    discovery: {
      ...state.discovery,
      refinement: { ...state.discovery.refinement, reviewPosture: posture },
    },
  };
};

/** Store the CEO review readiness score and optional reflection submitted in stage-b. */
export const setCeoReviewReadiness = (
  state: schema.StateFile,
  readiness: schema.CeoReviewReadiness,
  reflection?: string,
): schema.StateFile => {
  if (state.phase !== "DISCOVERY_REFINEMENT") {
    throw new Error(`setCeoReviewReadiness: invalid phase ${state.phase}`);
  }
  return {
    ...state,
    discovery: {
      ...state.discovery,
      refinement: {
        ...state.discovery.refinement,
        ceoReview: { readinessScore: readiness, reflection },
      },
    },
  };
};
...
/**
 * Clear the refinement sub-state — used by the "revise" answer in stage-c
 * to return the user to stage-a for a fresh completeness assessment.
 */
export const clearRefinement = (state: schema.StateFile): schema.StateFile => {
  if (state.phase !== "DISCOVERY_REFINEMENT") {
    throw new Error(`clearRefinement: invalid phase ${state.phase}`);
  }
  return {
    ...state,
    discovery: { ...state.discovery, refinement: undefined },
  };
};
...
/**
 * Import a detected plan file: stores its content as userContext and sets
 * planPath so buildPlanContext() will inject [STATED]/[INFERRED] instructions.
 * After this, hasPlan === true and the listen-first step no longer fires.
 */
export const importDetectedPlan = (
  state: schema.StateFile,
  content: string,
  planPath: string,
): schema.StateFile => ({
