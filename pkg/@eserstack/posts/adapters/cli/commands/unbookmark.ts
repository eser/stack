...
/**
 * `eser posts unbookmark <post-id> --platform=<platform>`
 *
 * @module
 */
...
export const main = async (
  ...
  if (postIdArg === undefined || platform === undefined) {
    await output.outputError(
      "Usage: eser posts unbookmark <post-id> --platform=<platform>",
    );
  ...
  const result = await task.runTask(
    cliTriggers.removeBookmark({ postId: postIdArg, platform }),
  );
  if (results.isFail(result)) {
    await output.outputError(result.error.message);
    return results.fail({ exitCode: 1 });
  }
  await output.outputSuccess(
    `Removed bookmark for ${postIdArg} on ${platform}`,
  );
